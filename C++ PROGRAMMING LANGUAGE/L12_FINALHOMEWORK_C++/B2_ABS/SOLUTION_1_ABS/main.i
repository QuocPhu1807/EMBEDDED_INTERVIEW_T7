...
void Car :: addMassOfPassengers(double massOfPassgenrs){
    ...
    this->totalMassOfCar = massOfPassgenrs + basicMassOfCar;
}

double Car ::getTotalMassOfCar(){

    return totalMassOfCar;
}
...
void ABS ::forceBrakeApplied(){
    ...
    else cerr<<endl<<" Warning: The wheel is locking up! Adjusting the brake force based on the mass of car: "<<(double)(getCar().getTotalMassOfCar())<<" (kg)";

    cerr<<endl<<" The speed of wheel now: "<<getCar().getWheel().getSpeedForWheel()<< " (rpm)";
...
int main(int argc, char const *argv[])
{
    ...
    checkOfAbs.getCar().getWheel().setSpeedForWheel(13);
    checkOfAbs.forceBrakeApplied();

    checkOfAbs.getCar().addMassOfPassengers(70);
    checkOfAbs.getCar().getWheel().setSpeedForWheel(9);
